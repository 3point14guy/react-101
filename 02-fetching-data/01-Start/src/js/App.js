...
class App extends React.Component {
  ...
  // componentDidMount is the react component for fetching info from 3rd party APIs
  componentDidMount(){
    ...
    if(tooOld){
      ...
      console.log(`Using data from localStorge that is ${dataAge} minutes old.`);
  }
      ...
      const {isLoading, contacts} = this.state;
        return (
            <div>
                <header>
                    <img src={image} />
                    <h1>Fetching Data <button className="btn btn-sm btn-danger" onClick={(e) => { this.fetchData();
                    }}>Fetch now</button></h1>
                </header>
